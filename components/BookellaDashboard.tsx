...
export default function BookellaDashboard() {
  ...
  // Bookella automated pricing logic (<30 -> 50, 30-39 -> +20, 40-50 -> +15, 60-77 -> +10)
  const calcPrice = (orig: number): number => {
    if (orig < 30) return 50;
    if (orig >= 30 && orig <= 39) return orig + 20;
  ...
  const handleLibraryImport = (libBook: {
    ...
    const origPrice = 50;
    const finalPrice = calcPrice(origPrice);
    ...
    showToast(`تم استيراد "${bTitle}" من ${libBook.source} بنجاح وتسعيره تلقائياً!`, "success");
  };
  ...
  const handleCsvImport = () => {
    ...
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        const finalPr = calcPrice(origPr);
  ...
  const handleExportCsv = () => {
    let header = "Book ID,الحرف,اسم الكتاب,نوع الكتاب,اسم الكاتب,التصنيف الرئيسي,اسم السلسلة,تصنيفات فرعية,Original Price,Price,الكمية المتاحة,لغة الكتاب,دار النشر,عدد الصفحات,ISBN,رابط صورة الغلاف,ملاحظات,اسم المكتبة\n";
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 shadow-2xl animate-in fade-in-50 zoom-in-95 border border-[#E1E2EC]">
            ...
            <form onSubmit={handleAddBook} className="space-y-4">
              ...
              <div className="bg-[#F8F9FF] border border-[#E1E2EC] p-4 rounded-2xl flex items-center justify-between">
                <div>
                  <label htmlFor="book-original-price" className="block text-xs font-bold text-[#44474E] mb-1">
                    السعر الأصلي (ج.م)
                  </label>
                  <input 
                    ...
                    aria-label="السعر الأصلي بالجنيه المصري"
                    placeholder="40"
        ...
        <div className="fixed inset-0 z-50 bg-[#001D35]/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-5xl h-[88vh] p-8 shadow-2xl animate-in fade-in-50 zoom-in-95 border border-violet-100 flex flex-col overflow-hidden relative">
            ...
            <div className="flex items-center justify-between border-b border-[#E1E2EC] pb-4 mb-5 shrink-0">
              ...
              <button onClick={() => setIsLibrarySearchOpen(false)} className="text-stone-400 hover:text-stone-600 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
